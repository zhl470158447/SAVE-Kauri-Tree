...
public class Generator : MonoBehaviour {
	...
	[Range(0f, 1f)]
	public float _timeBetweenIterations = 0.0f;
	[Range(0f, 3f)]
	...
	public float _branchingAngle = 0.0f;
	...
	void Start () {
		...
		//GenerateAttractorsRoots(_nbAttractors, _radius);
				...
				if (_activeAttractors.Count != 0) {
					...
					foreach (Branch b in _branches) {
						...
						if (b._attractors.Count > 0) {
							...
							Matrix4x4 m = Matrix4x4.Rotate(Quaternion.Euler(_branchingAngle,0f,0f));
							dir += m.MultiplyVector(b._direction);
							dir.Normalize();
